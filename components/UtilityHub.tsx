  ...
  return <div className="mx-auto w-full max-w-4xl space-y-6 pb-6 text-slate-900">
    ...
    {latest && <dialog ref={dialog} aria-labelledby="utility-review-title" onCancel={e => {e.preventDefault();if (!busy) setSelected(null)}} className="m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-lg overflow-y-auto rounded-3xl border-0 bg-white p-0 text-slate-900 shadow-xl backdrop:bg-slate-950/50">
      ...
      <div className="space-y-5 p-6">
        ...
        {selectedResponsibility !== "SHARED" && (latest.collected_amount_paise || 0) > 0 && <section className="grid grid-cols-2 gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm"><div><p className="text-slate-500">Already collected</p><p className="mt-1 font-semibold">{utilityMoney(latest.collected_amount_paise || 0)}</p></div><div><p className="text-slate-500">Still to collect</p><p className="mt-1 font-semibold">{utilityMoney(selectedResponsibility === "LANDLORD_PAYS" ? landlordRemaining : tenantRemaining)}</p></div></section>}
