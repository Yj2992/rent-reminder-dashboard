...
import { currentUtilityBills, HubAccount, HubBill, Service, utilityBillCopy, UtilityPayerRole, utilityCheckedAt, utilityDate, utilityMoney, utilitySharePaise, utilityShareRemaining, utilityStage } from "../lib/utilityUi"
...
const progressStep = (bill: HubBill) => utilityStage(bill) === "paid" ? 3 : utilityStage(bill) === "processing" ? 2 : (bill.collected_amount_paise || 0) > 0 ? 1 : 0
export default function UtilityHub({ accounts, bills, loading = false, error, onRefresh, onPay, onReceipt, onAdd, onEdit, onFetch, onRemind, canPay, portalRole = "OWNER" }: {
  ...
  function billButton(bill: HubBill) {
    ...
    return <button key={bill.id} type="button" onClick={() => setSelected(bill)} className="flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-blue-300 hover:bg-blue-50/30">
      ...
      <span className="shrink-0 text-right"><span className="block text-sm font-semibold text-slate-900">{utilityMoney(bill.bill_amount_paise)}</span><span className={"mt-1 block text-xs " + (stage === "paid" ? "text-emerald-700" : stage === "due" ? "text-blue-700" : "text-amber-700")}>{utilityBillCopy(bill).label}</span></span>
    </button>
  }
  return <div className="mx-auto w-full max-w-4xl space-y-6 pb-6 text-slate-900">
    <div className="flex items-center justify-between gap-4"><div><h2 className="text-2xl font-semibold tracking-tight">Pay utility bills</h2><p className="mt-1 text-sm text-slate-500">Choose a service, review the latest provider amount, and pay securely.</p></div><button className={secondary} disabled={loading || !!busy} onClick={() => void onRefresh()}>{loading ? "Refreshing…" : "Refresh"}</button></div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex gap-1 rounded-full bg-slate-100 p-1" aria-label="Utility views">{([["accounts", "Home"], ["activity", "Activity"], ["receipts", "Receipts"]] as const).map(([key, label]) => <button key={key} aria-pressed={view === key} onClick={() => setView(key)} className={"rounded-full px-3 py-2 text-xs font-semibold sm:text-sm " + (view === key ? "bg-white text-blue-700 shadow-sm" : "text-slate-600")}>{label}</button>)}</div>
    ...
    {loading && !accounts.length && !bills.length ? <div role="status" className="rounded-3xl bg-white p-10 text-center text-sm text-slate-500">Loading your utility accounts…</div> : view === "accounts" ? <>
      <div className="space-y-3">{visibleAccounts.slice(0, limit).map(account => {
        ...
        const canRefreshProvider = !bill || stage === "paid" || stage === "cancelled" || (["due", "checking", "review"].includes(stage || "") && !["ENQUEUED", "WAITING_FOR_FLOAT", "PROCESSING", "RECONCILIATION_REQUIRED"].includes(bill.bbps_status || ""))
        return <article key={account.id} className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
            <div>{bill ? <><p className="text-lg font-semibold">{utilityMoney(bill.bill_amount_paise)}</p><p className="mt-0.5 text-xs text-slate-500">{utilityBillCopy(bill).label}{stage === "due" ? " · Due " + utilityDate(bill.due_date) : ""}</p><p className="mt-1 text-xs text-slate-400">{utilityCheckedAt(bill.provider_checked_at || bill.updated_at || bill.created_at)}</p></> : <p className="text-sm text-slate-500">No bill fetched yet</p>}</div>
            <div className="flex flex-wrap gap-2">{onFetch && canRefreshProvider && <button disabled={!!busy || loading} onClick={() => void act(account.id, () => onFetch(account))} className={bill ? secondary : primary}>{busy === account.id ? "Checking Eko…" : !bill ? "Fetch bill" : ["due", "checking", "review"].includes(stage || "") ? "Refresh bill" : "Fetch next bill"}</button>}
    ...
    {latest && <dialog ref={dialog} aria-labelledby="utility-review-title" onCancel={e => {e.preventDefault();if (!busy) setSelected(null)}} className="m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-lg overflow-y-auto rounded-3xl border-0 bg-white p-0 text-slate-900 shadow-xl backdrop:bg-slate-950/50">
      ...
      <div className="space-y-5 p-6">
        <div className="text-center"><span className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 text-blue-700"><ServiceIcon type={selectedAccount?.utility_type}/></span><p className="font-semibold">{selectedAccount?.operator_name || "Utility bill"}</p><p className="mt-3 text-3xl font-semibold tracking-tight">{utilityMoney(latest.bill_amount_paise)}</p><p className="mt-2 text-sm text-slate-500">{utilityBillCopy(latest).label}{(latest.collected_amount_paise || 0) > 0 ? ` · ${utilityMoney(latest.collected_amount_paise || 0)} customer paid` : ""}</p></div>
        <ol aria-label="Payment progress" className="grid grid-cols-4 gap-1 text-center text-[10px] text-slate-500 sm:text-xs">{["Bill ready", "Money received", "At provider", "Paid"].map((label,index)=><li key={label} className={index <= progressStep(latest) ? "font-semibold text-blue-700" : ""}><span className={"mx-auto mb-2 block h-2.5 w-2.5 rounded-full " + (index <= progressStep(latest) ? "bg-blue-700" : "bg-slate-200")}/>{label}</li>)}</ol>
        ...
        <p className="text-sm leading-6 text-slate-600">{utilityBillCopy(latest).detail}</p>
        ...
        {selectedResponsibility !== "SHARED" && (latest.collected_amount_paise || 0) > 0 && <section className="grid grid-cols-2 gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm"><div><p className="text-slate-500">Customer paid</p><p className="mt-1 font-semibold">{utilityMoney(latest.collected_amount_paise || 0)}</p></div><div><p className="text-slate-500">{latest.owner_funded_paise ? "Advanced by Rentomatic" : "Still to collect"}</p><p className="mt-1 font-semibold">{utilityMoney(latest.owner_funded_paise || (selectedResponsibility === "LANDLORD_PAYS" ? landlordRemaining : tenantRemaining))}</p></div></section>}
        ...
        {utilityStage(latest) === "paid" && <button className={primary + " w-full"} disabled={!!busy} onClick={() => void act(latest.id, () => onReceipt(latest))}>{busy ? "Downloading…" : "Download PDF receipt"}</button>}
