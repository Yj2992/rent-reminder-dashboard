      ...
      <div className="space-y-3">{visibleAccounts.slice(0, limit).map(account => {
        ...
        return <article key={account.id} className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
            ...
            <div className="flex flex-wrap gap-2">{onFetch && (!bill || stage === "paid" || stage === "cancelled") && <button disabled={!!busy || loading} onClick={() => void act(account.id, () => onFetch(account))} className={bill ? secondary : primary}>{busy === account.id ? "Fetching…" : bill ? "Fetch next bill" : "Fetch bill"}</button>}
